...
- Exact HTTP codes are absent, but the written business outcome supports an HTTP-class oracle: successful documented updates expect `2xx`; authentication, authorization, validation, route, and missing-target failures expect `4xx`. Where acceptance policy itself is undocumented, the expected status is an explicit `2xx/4xx` characterization branch. A concrete code such as `200`, `400`, `401`, `403`, or `404` must not be asserted without an implementation contract.
...
| PC-DESC | `/api/products/:id` | body | `description` | JSON string in example | Contract gap — requiredness is not documented | No requiredness, minimum/maximum length, normalization, or content rule | `Updated description` | API spec §3.3 | String/omitted/null/wrong-type representatives covered; no BVA supported |
| PC-IMAGE | `/api/products/:id` | body | `imageUrl` | JSON string in example | Contract gap — requiredness is not documented | Name/example suggest a URL, but no URL-format, scheme, requiredness, empty, or length rule is stated | `https://example.test/updated.png` | API spec §3.3 | URL-shaped/non-URL/empty/omitted/null/wrong-type representatives covered; no BVA supported |
...
| PC-EXTRA | `/api/products/:id` | body | additional properties | JSON members | Contract gap — additional-property policy is not documented | Contract shows five fields but does not define strict versus lenient handling | none | API spec §3.3 | Documented-only and unknown-field partitions covered |
...
| PC-RESP | `/api/products/:id` | response | status/body/schema | HTTP class inferable; exact code/body schema absent | Response body requiredness is a contract gap | Success requires a `2xx` class; required rejection requires a `4xx` class; body, media type, and exact code are undocumented | Record actual response; validate only the applicable class and documented side effect | README §FR-12/FR-15; API spec §3.3 | HTTP-class oracle completed; exact response schema deferred to Security/Schema Checklist |
...
| NAME-V2 | PC-NAME | `name` | content | Whitespace-only string is present and within the only documented length limit; no trim/nonblank rejection rule exists | Yes by the written rule; normalization remains a contract gap | `"   "` | README §FR-15 |
...
| IMAGE-V2 | PC-IMAGE | `imageUrl` | format | Non-URL string; the documented contract requires only the shown string representation and states no URL grammar | Yes by the documented representation; UI suitability remains separate | `not-a-url` | API spec §3.3 |
| IMAGE-V3 | PC-IMAGE | `imageUrl` | content | Empty string; no non-empty constraint is documented | Yes by the documented representation | `""` | API spec §3.3 |
...
| RESP-U1 | PC-RESP | response | contract | Exact status, success body, error body, and schema are absent | HTTP class is inferable per scenario; exact code/schema is a contract gap | record actual redacted response and compare its class with the scenario oracle | README §FR-12/FR-15; API spec §3.3 |
...
Unless a row says otherwise, use endpoint `PUT /api/products/101`, headers `H0`, body `B0`, existing `P1`, `C1`, and an immutable `P2+` pre-state. `B0 with X` changes only `X`. Exact numeric status and response schema remain unresolved; valid cases require a `2xx` response and the documented business update, required rejections require a `4xx` response and no mutation, and characterization cases accept only the stated `2xx/4xx` branch plus its matching state oracle.
...
|---|---|---|---|---|---|---|---|---|---|
| FR15-DOM-001 | `/api/products/101` | PUT | Baseline valid admin update using all documented fields | `A1`, `P1`, `C1`, snapshot `P2+` | `B0` | `2xx Success` — exact code not documented | `P1` receives exactly the accepted values; every `P2+` record remains unchanged; response schema unspecified | ENV-V1, ID-V1, AUTH-V1, ROLE-V1, CTYPE-V1, NAME-V1, PRICE-V1, DESC-V1, IMAGE-V1, CAT-V1, EXTRA-V1, TARGET-V1, OTHERS-V1 | README §FR-12/FR-15; API spec §3.3 |
| FR15-DOM-002 | `/api/products/101` | PUT | Reject an absent request body | Baseline except body | `<absent>` | `4xx Client Error` — exact code not documented | Required product inputs are absent; no product changes | ENV-I1 | README §FR-15; API spec §3.3 |
| FR15-DOM-003 | `/api/products/101` | PUT | Reject malformed JSON | Baseline | raw `{"name":` | `4xx Client Error` — exact code not documented | Body cannot supply valid fields; no product changes; error schema unspecified | ENV-I2 | API spec §3.3 |
| FR15-DOM-004 | `/api/products/101` | PUT | Reject a non-object JSON body | Baseline | `[]` | `4xx Client Error` — exact code not documented | Body does not match documented object representation; no product changes | ENV-I3 | API spec §3.3 |
| FR15-DOM-005 | `/api/products/999999` | PUT | Characterize a well-formed-looking but non-existent product identifier | `A1`; `999999` resolves to no product; snapshot all products | `B0` | `4xx Not-found-class response` — exact code not documented | No existing product may change; exact not-found status/body unspecified | ID-U1, OTHERS-V1 | API spec §3.3 |
| FR15-DOM-006 | `/api/products/abc` | PUT | Characterize opaque/non-numeric `id` when identifier syntax is undocumented | `A1`; snapshot all products | `B0` | `4xx Client/Route Error` — exact code not documented | Record behavior; request must not be redirected to or mutate any existing product | ID-U2, OTHERS-V1 | API spec §3.3 |
| FR15-DOM-007 | `/api/products/` | PUT | Exercise an omitted required path segment as a negative route-template instantiation | `A1`; snapshot all products | `B0`; no `:id` segment | `4xx Route Error` — exact code not documented | No product may change; this is not a separate API selection, only the missing-path-parameter partition | ID-I1, OTHERS-V1 | API spec §3.3 |
| FR15-DOM-008 | `/api/products/101` | PUT | Reject missing authentication | `P1`, `C1`, snapshot `P2+`; no auth header | `B0` | `4xx Authentication Error` — exact code not documented | No product changes; exact authentication status/body unspecified | AUTH-I1 | README §FR-12; §SEC-02 |
| FR15-DOM-009 | `/api/products/101` | PUT | Reject an empty bearer token | Baseline except `Authorization: Bearer ` | `B0` | `4xx Authentication Error` — exact code not documented | No product changes | AUTH-I2 | README §FR-12; §SEC-02 |
| FR15-DOM-010 | `/api/products/101` | PUT | Reject a non-Bearer authorization scheme | Baseline except `Authorization: Basic abc` | `B0` | `4xx Authentication Error` — exact code not documented | No product changes | AUTH-I3 | README §FR-02/FR-12 |
| FR15-DOM-011 | `/api/products/101` | PUT | Reject a JWT with invalid signature | Baseline except tampered JWT | `B0` | `4xx Authentication Error` — exact code not documented | No product changes | AUTH-I4 | README §FR-12; §SEC-02 |
| FR15-DOM-012 | `/api/products/101` | PUT | Reject an expired JWT | Baseline except expired JWT | `B0` | `4xx Authentication Error` — exact code not documented | No product changes | AUTH-I5 | README §FR-12; §SEC-02 |
| FR15-DOM-013 | `/api/products/101` | PUT | Reject a valid JWT whose role is not admin | Use `U1`; `P1`, `C1`, snapshot `P2+` | `B0` | `4xx Authorization Error` — exact code not documented | No product changes; token existence alone must not authorize update | ROLE-I1 | README §FR-12; §SEC-03 |
| FR15-DOM-014 | `/api/products/101` | PUT | Characterize omitted `Content-Type` | Baseline auth/state; omit header while sending JSON bytes | bytes for `B0` | `2xx Success` if parsed; otherwise `4xx Client Error` — media policy unresolved | If rejected, no product changes; if accepted by parser, only `P1` changes correctly; final verdict needs confirmed media policy | CTYPE-U1 | API spec §3.3 |
| FR15-DOM-015 | `/api/products/101` | PUT | Characterize JSON-looking bytes labeled `text/plain` | Baseline except `Content-Type: text/plain` | bytes for `B0` | `2xx Success` if parsed; otherwise `4xx Client Error` — parser policy unresolved | Record behavior; rejection leaves all products unchanged, while acceptance may change only `P1` | CTYPE-U2 | API spec §3.3 |
| FR15-DOM-016 | `/api/products/101` | PUT | Test `name` length `LB-1 = 0` | Baseline | `B0` with `name:""` | `4xx Validation Error` — exact code not documented | No product changes | NAME-I1; BVA `LB-1` | README §FR-15 |
| FR15-DOM-017 | `/api/products/101` | PUT | Test minimum valid `name` length `LB = 1` | Baseline | `B0` with `name:N(1)` | `2xx Success` — exact code not documented | `P1.name` becomes the 1-character value; `P2+` unchanged | NAME-V1; BVA `LB` | README §FR-15 |
| FR15-DOM-018 | `/api/products/101` | PUT | Test `name` length `LB+1 = 2` | Baseline | `B0` with `name:N(2)` | `2xx Success` — exact code not documented | `P1.name` becomes the 2-character value; `P2+` unchanged | NAME-V1; BVA `LB+1` | README §FR-15 |
| FR15-DOM-019 | `/api/products/101` | PUT | Test `name` length `UB-1 = 254` | Baseline | `B0` with `name:N(254)` | `2xx Success` — exact code not documented | `P1.name` becomes the 254-character value; `P2+` unchanged | NAME-V1; BVA `UB-1` | README §FR-15 |
| FR15-DOM-020 | `/api/products/101` | PUT | Test maximum valid `name` length `UB = 255` | Baseline | `B0` with `name:N(255)` | `2xx Success` — exact code not documented | `P1.name` becomes the 255-character value; `P2+` unchanged | NAME-V1; BVA `UB` | README §FR-15 |
| FR15-DOM-021 | `/api/products/101` | PUT | Test `name` length `UB+1 = 256` | Baseline | `B0` with `name:N(256)` | `4xx Client Error` — exact code not documented | No product changes | NAME-I2; BVA `UB+1` | README §FR-15 |
| FR15-DOM-022 | `/api/products/101` | PUT | Reject omitted required `name` | Baseline | `B0` without `name` | `4xx Client Error` — exact code not documented | No product changes | NAME-I3 | README §FR-15 |
| FR15-DOM-023 | `/api/products/101` | PUT | Reject null `name` | Baseline | `B0` with `name:null` | `4xx Client Error` — exact code not documented | No product changes | NAME-I4 | README §FR-15; API spec §3.3 |
| FR15-DOM-024 | `/api/products/101` | PUT | Reject non-string `name` | Baseline | `B0` with `name:123` | `4xx Client Error` — exact code not documented | No product changes | NAME-I5 | README §FR-15; API spec §3.3 |
| FR15-DOM-025 | `/api/products/101` | PUT | Accept whitespace-only `name` under the documented presence/length rules | Baseline | `B0` with `name:"   "` | `2xx Success` — whitespace is within the documented 1–255-character domain; exact code not documented | Only `P1.name` becomes the submitted three-character value; `P2+` remains unchanged. Any trimming behavior is recorded as an unresolved normalization policy | NAME-V2 | README §FR-15 |
| FR15-DOM-026 | `/api/products/101` | PUT | Test price below the strict lower threshold | Baseline | `B0` with `price:-1` | `4xx Client Error` — exact code not documented | No product changes | PRICE-I1; lower-bound below probe | README §FR-15 |
| FR15-DOM-027 | `/api/products/101` | PUT | Test price exactly at excluded threshold 0 | Baseline | `B0` with `price:0` | `4xx Client Error` — exact code not documented | No product changes | PRICE-I1; lower-bound threshold | README §FR-15 |
| FR15-DOM-028 | `/api/products/101` | PUT | Test conventional just-above positive price | Baseline | `B0` with `price:1` | `2xx Success` — exact code not documented | `P1.price` becomes `1`; `P2+` unchanged | PRICE-V1; lower-bound above probe | README §FR-15 |
| FR15-DOM-029 | `/api/products/101` | PUT | Test positive fractional number when integer/scale restriction is absent | Baseline | `B0` with `price:0.01` | `2xx Success` — exact code not documented | Only `P1.price` changes; exact storage precision/rounding is unspecified and must be recorded | PRICE-V2; fractional precision probe | README §FR-15; API spec §3.3 |
| FR15-DOM-030 | `/api/products/101` | PUT | Reject omitted required `price` | Baseline | `B0` without `price` | `4xx Client Error` — exact code not documented | No product changes | PRICE-I2 | README §FR-15 |
| FR15-DOM-031 | `/api/products/101` | PUT | Reject null `price` | Baseline | `B0` with `price:null` | `4xx Client Error` — exact code not documented | No product changes | PRICE-I3 | README §FR-15; API spec §3.3 |
| FR15-DOM-032 | `/api/products/101` | PUT | Reject numeric-looking string `price` | Baseline | `B0` with `price:"150000"` | `4xx Client Error` — exact code not documented | No product changes; no undocumented string-to-number coercion oracle is assumed | PRICE-I4 | README §FR-15; API spec §3.3 |
| FR15-DOM-033 | `/api/products/101` | PUT | Reject Boolean `price` | Baseline | `B0` with `price:true` | `4xx Client Error` — exact code not documented | No product changes | PRICE-I5 | README §FR-15; API spec §3.3 |
| FR15-DOM-034 | `/api/products/101` | PUT | Accept a Vietnamese Unicode description string | Baseline | `B0` with `description:"Mô tả sản phẩm cập nhật"` | `2xx Success` — exact code not documented | Only `P1.description` changes; encoding normalization is unspecified; `P2+` unchanged | DESC-V1 | API spec §3.3 |
| FR15-DOM-035 | `/api/products/101` | PUT | Exercise empty description with no non-empty rule | Baseline | `B0` with `description:""` | `2xx Success` — exact code not documented | Only `P1.description` becomes empty; `P2+` unchanged | DESC-V1 | API spec §3.3 |
| FR15-DOM-036 | `/api/products/101` | PUT | Characterize omitted `description` | Baseline | `B0` without `description` | `2xx Success` if omission is allowed; otherwise `4xx Client Error` — policy unresolved | Record whether value is preserved, cleared, or request rejected; only `P1` may differ and final verdict needs confirmation | DESC-U1 | API spec §3.3 |
| FR15-DOM-037 | `/api/products/101` | PUT | Characterize null `description` | Baseline | `B0` with `description:null` | `2xx Success` if `null` is allowed; otherwise `4xx Client Error` — policy unresolved | Record behavior; rejection must not mutate products; acceptance semantics need human confirmation | DESC-U2 | API spec §3.3 |
| FR15-DOM-038 | `/api/products/101` | PUT | Characterize non-string `description` | Baseline | `B0` with `description:{}` | `2xx Success` if coercion is implemented; otherwise `4xx Client Error` — policy unresolved | Record behavior; no non-target mutation; do not invent coercion expectations | DESC-U3 | API spec §3.3 |
| FR15-DOM-039 | `/api/products/101` | PUT | Accept a non-URL string because no URL grammar is documented | Baseline | `B0` with `imageUrl:"not-a-url"` | `2xx Success` — no URL-format rule is documented; exact code not documented | Only `P1.imageUrl` becomes the submitted literal; `P2+` remains unchanged. UI rendering/sink safety is tested separately | IMAGE-V2 | API spec §3.3 |
| FR15-DOM-040 | `/api/products/101` | PUT | Accept empty `imageUrl` because no non-empty rule is documented | Baseline | `B0` with `imageUrl:""` | `2xx Success` — no non-empty rule is documented; exact code not documented | Only `P1.imageUrl` becomes empty; `P2+` remains unchanged | IMAGE-V3 | API spec §3.3 |
| FR15-DOM-041 | `/api/products/101` | PUT | Characterize omitted `imageUrl` | Baseline | `B0` without `imageUrl` | `2xx Success` if omission is allowed; otherwise `4xx Client Error` — policy unresolved | Record whether value is preserved, cleared, or request rejected; `P2+` unchanged | IMAGE-U3 | API spec §3.3 |
| FR15-DOM-042 | `/api/products/101` | PUT | Characterize null `imageUrl` | Baseline | `B0` with `imageUrl:null` | `2xx Success` if `null` is allowed; otherwise `4xx Client Error` — policy unresolved | Record behavior; rejection leaves all products unchanged; accepted semantics need confirmation | IMAGE-U4 | API spec §3.3 |
| FR15-DOM-043 | `/api/products/101` | PUT | Characterize non-string `imageUrl` | Baseline | `B0` with `imageUrl:123` | `2xx Success` if coercion is implemented; otherwise `4xx Client Error` — policy unresolved | Record behavior; no non-target mutation | IMAGE-U5 | API spec §3.3 |
| FR15-DOM-044 | `/api/products/101` | PUT | Update target to a different existing category | `A1`, `P1`, both `C1` and `C2`, snapshot `P2+` | `B0` with `category_id:2` | `2xx Success` — exact code not documented | `P1.category_id` becomes `2`; all `P2+` records unchanged | CAT-V1 | README §FR-15; API spec §3.3 |
| FR15-DOM-045 | `/api/products/101` | PUT | Reject category ID that does not select an existing category | `A1`, `P1`, `C404` confirmed absent, snapshot `P2+` | `B0` with `category_id:999999` | `4xx Client Error` — exact code not documented | No product changes | CAT-I1 | README §FR-15 |
| FR15-DOM-046 | `/api/products/101` | PUT | Reject omitted required `category_id` | Baseline | `B0` without `category_id` | `4xx Client Error` — exact code not documented | No product changes | CAT-I2 | README §FR-15 |
| FR15-DOM-047 | `/api/products/101` | PUT | Reject null `category_id` | Baseline | `B0` with `category_id:null` | `4xx Client Error` — exact code not documented | No product changes | CAT-I3 | README §FR-15; API spec §3.3 |
| FR15-DOM-048 | `/api/products/101` | PUT | Reject numeric-looking string `category_id` | Baseline | `B0` with `category_id:"1"` | `4xx Validation Error` — exact code not documented | No product changes; no undocumented coercion assumed | CAT-I4 | README §FR-15; API spec §3.3 |
| FR15-DOM-049 | `/api/products/101` | PUT | Reject fractional `category_id` | Baseline | `B0` with `category_id:1.5` | `4xx Validation Error` — exact code not documented | No product changes | CAT-I5 | README §FR-15; API spec §3.3 |
| FR15-DOM-050 | `/api/products/101` | PUT | Characterize one unknown body property | Baseline | `B0` plus `"unexpected_field":"sentinel"` | `2xx Success` if ignored; otherwise `4xx Client Error` — policy unresolved | Reject unchanged or ignore the member and update only documented fields on `P1`; the unknown member must not alter another product/protected state | EXTRA-U1 | API spec §3.3 |
...
- HTTP-status completion: all 50 cases now have a source-supported `2xx`, `4xx`, or explicit `2xx/4xx` branch oracle; every status cell is actionable at the precision supported by the documents.
- Human confirmation is required before later automation finalizes: `id` syntax and exact not-found code; `name` normalization/trimming; positive fractional-price scale/rounding; `description` and `imageUrl` omission/null/type semantics; Content-Type behavior; unknown-property policy; every exact status/response schema; and the allowed non-GET state oracle.
