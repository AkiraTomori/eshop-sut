...
- Symbols in effect rows: `X` = effect required by the cited specification; `?` = policy-dependent effect/outcome requiring user confirmation because the source contract does not document it; `—` = effect does not apply. Every cause cell in the full table remains `Y` or `N` as required.

- If a request is rejected, the integrity oracle is no order creation and no cart clearing. The expected HTTP class is still asserted below; the exact numeric rejection code, response body, and precedence among simultaneous errors are not documented.
...
| C2 | Cause | Authenticated user's cart is empty | `Y` = zero items; `N` = at least one item | Cart is the total source in README §FR-08; empty-cart outcome is not documented |

| C3 | Cause | Client `total_amount` matches server-computed cart total `C` | `Y` = `T=C`; `N` = `T!=C` | README §FR-08; API spec §4.3 |

| C4 | Cause | `shipping_address` is structurally valid under the working classifier | `Y` = non-empty string; `N` = `null` | String example in API spec §4.3; full validity rule is not documented |
...
| E2 | Effect | Reject empty-cart checkout | Candidate effect only; actual policy and precedence are not documented | Contract gap in README §FR-08 and API spec §4.3 |

| E3 | Effect | Reject invalid `shipping_address` | Candidate effect only; exact validity rule and precedence are not documented | String example only in API spec §4.3 |
...
| E5 | Effect | Create an order for the authenticated user | Observable persistent side effect; response schema is not documented | Checkout/order creation implied by README §FR-08 and API spec §4.3 |
...
| E7 | Effect | Preserve order/cart state after rejection | No order created and original cart retained | Integrity consequence of rejected checkout; exact atomicity contract is not documented |
...
| R2 | `Y/N/Y/N` | Address rejection/acceptance and error precedence are not documented | Keep separate |

| R3 | `Y/N/N/Y` | Mismatching total cannot control outcome; backend uses `C`, creates order, clears cart | Merge with R1 |

| R4 | `Y/N/N/N` | Both total mismatch and invalid address are present; address behavior/precedence is not documented | Keep separate from R2 because precedence is undocumented |

| R5 | `Y/Y/Y/Y` | Empty-cart behavior is not documented even with otherwise valid inputs | Keep separate |

| R6 | `Y/Y/Y/N` | Empty cart plus invalid address; neither outcome nor precedence is documented | Keep separate |

| R7 | `Y/Y/N/Y` | Empty cart plus mismatching client total; outcome is not documented, while the non-trust invariant still applies if accepted | Keep separate |

| R8 | `Y/Y/N/N` | Empty cart, mismatching total, invalid address; outcome and precedence are not documented | Keep separate |
...
`C` always denotes the disposable server-side cart total. Each case uses `POST /api/checkout`; later fixture setup/observation must not add a GET request. For a policy-dependent branch, the case records behavior for human review while enforcing only the invariant: rejection preserves state; acceptance uses `C` and clears the cart.
...
| FR08-DT-001 | RR-AUTH | `POST /api/checkout` | Prepare a non-empty `U1` cart with `C=250000`; omit or invalidate the request JWT | `T=250000`; non-empty string address | `4xx Authentication Error` — exact code not documented | Authentication rejection; no order created; prepared cart remains unchanged. Represents R9–R16 because all other causes are security-irrelevant without valid auth | README §FR-08/§SEC-02; API spec §4 |

| FR08-DT-002 | RR-SUCCESS | `POST /api/checkout` | Valid `U1` JWT; non-empty cart with `C=250000` | `T=1` (`T!=C`); `shipping_address:"123 Le Loi, TP.HCM"` | `2xx Success` — exact code not documented | Backend distrusts `T`, creates order total `250000`, then clears `U1` cart. One concrete mismatching representative covers reduced C3 `-`; matching baseline already exists in Domain case FR08-DOM-001 | README §FR-08 |

| FR08-DT-003 | RR-ADDR-MATCH | `POST /api/checkout` | Valid `U1` JWT; non-empty cart with `C=250000` | `T=250000`; `shipping_address:null` | `4xx Client/Schema Validation Error` or `2xx Success` — address policy and exact code not documented | If rejected: no order/cart change. If accepted: order total must be `250000` and the cart must clear. Record the observed branch for human confirmation | API spec §4.3; address rule absent from README §FR-08 |

| FR08-DT-004 | RR-ADDR-MISMATCH | `POST /api/checkout` | Valid `U1` JWT; non-empty cart with `C=250000` | `T=1`; `shipping_address:null` | `4xx Client/Schema Validation Error` or `2xx Success` — address policy and exact code not documented | Rejection preserves state; acceptance must ignore `T`, create total `250000`, and clear the cart. Record the observed branch for human confirmation | README §FR-08; API spec §4.3 |

| FR08-DT-005 | RR-EMPTY-MATCH-VALID | `POST /api/checkout` | Valid `U1` JWT; empty cart (`C=0`) | `T=0`; non-empty string address | `2xx Success` or `4xx Empty-cart Client Error` — policy and exact code not documented | If accepted, create an order with server total `0` and retain an empty cart; if rejected, create no order and keep the cart unchanged. Record the observed branch for human confirmation | Contract gap in README §FR-08/API spec §4.3 |

| FR08-DT-006 | RR-EMPTY-MATCH-INVALID | `POST /api/checkout` | Valid `U1` JWT; empty cart (`C=0`) | `T=0`; `shipping_address:null` | `4xx Client Error` or `2xx Success` — empty-cart/address policies, precedence, and exact code not documented | If rejected, create no order and keep the cart empty. If accepted, use server total `0` and retain an empty cart. Record the observed branch and any error precedence | Contract gaps in README §FR-08/API spec §4.3 |

| FR08-DT-007 | RR-EMPTY-MISMATCH-VALID | `POST /api/checkout` | Valid `U1` JWT; empty cart (`C=0`) | `T=1`; non-empty string address | `2xx Success` or `4xx Empty-cart Client Error` — policy and exact code not documented | If accepted, backend must ignore `T`, create total `C=0`, and retain an empty cart; if rejected, no order/cart mutation. Record the observed branch for human confirmation | README §FR-08; empty-cart policy absent |

| FR08-DT-008 | RR-EMPTY-MISMATCH-INVALID | `POST /api/checkout` | Valid `U1` JWT; empty cart (`C=0`) | `T=1`; `shipping_address:null` | `4xx Client Error` or `2xx Success` — empty-cart/address policies, precedence, and exact code not documented | If rejected, create no order and keep the cart empty. If accepted, use `C=0`, never `T=1`, and retain an empty cart. Record the observed branch and any error precedence | README §FR-08; API spec §4.3 contract gaps |
...
- Rules involving an empty cart or structurally invalid address are not over-reduced. Their acceptance, error response, and precedence are not documented by the source contract.

- Expected Status is complete for all eight derived cases: fixed authentication/success outcomes use the appropriate class (`4xx` or `2xx`), while address-validity and empty-cart contract gaps retain explicit `2xx`/`4xx` branches for human confirmation. Exact numeric codes and response schemas remain contract gaps; no `400`, `401`, `422`, or exact success code is guessed.
