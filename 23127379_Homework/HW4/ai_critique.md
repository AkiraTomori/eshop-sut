...
**The second notable failure was incomplete assertion coverage.** For FR-06's Add-to-Cart test, the AI only asserted that a toast appeared (`toBeVisible`), missing the more valuable assertions: toast message text, cart badge count, and cart state via API. The AI optimised for "the test passes" rather than "the test catches regression". This bias likely stems from its tendency to generate plausible-looking code rather than adversarially thorough test logic.
