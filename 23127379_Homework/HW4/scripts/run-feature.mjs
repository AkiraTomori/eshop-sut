...
const hw4Dir = path.resolve(scriptDir, '..');
...
for (const browser of ['chromium', 'firefox', 'webkit']) {
  const reportDir = path.join(feature.pool, 'playwright-report', browser);
  const resultsDir = path.join(feature.pool, 'test-results', browser);
  const result = spawnSync(
    ...
    [playwrightCli, 'test', feature.spec, '--project', browser],
    {
      ...
      env: {
        ...
        HW4_RESULTS_DIR: resultsDir,
        HW4_JSON_OUTPUT_FILE: path.join(resultsDir, 'results.json'),
  ...
  if (result.status !== 0) anyRunFailed = true;
}

if (anyRunFailed) process.exit(1);
