...
Read `AGENTS.md` completely, then read `.agents/skills/api-skill/cicd-pipeline-generator/SKILL.md` completely. Use only user-approved collection, environment, and data artifacts. If they are unavailable, emit explicit `INCOMPLETE` placeholders instead of inventing paths or commands.
...
Stop after presenting the proposals and wait for user review. This command does not advance a pool stage. Append exactly one redacted row for this AI invocation to `.agents/skills/api-skill/state/ai-audit-log.md` using the AI Audit Report format.
