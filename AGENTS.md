...
This file is the single source of truth for AI orchestration in CSC13003 HW06 API Testing. Tool-neutral command templates live in `.agents/commands/`, and both commands and skills must follow this file. `README.md` remains the source for SRS/business rules, and `api_specification.md` remains the source for endpoint contracts. When an expected status, field, or behavior is absent from those specifications, mark it as unspecified and request user confirmation instead of inventing it.
...
3. **Coverage:** Produce at least 35 reviewed test cases for each API unit. Use the taught techniques in separate steps; never generate an entire API suite with one undifferentiated prompt.
4. **Specification grounding:** Derive constraints and oracles from `README.md` and `api_specification.md`. Preserve FR/SEC/API traceability and label unspecified contracts explicitly.
5. **Human confirmation gate:** Every generated output is a proposal. Stop after each stage and wait for the exact confirmation `confirm stage N`. Never mark the stage complete, alter user labels, advance the pipeline, change pools, post an issue, commit, or trigger the next command before confirmation.
...
8. **Sequential workflow:** Complete Pools A → B → C. Within a pool, complete Stages 1 → 2 → 3 → 4 → 5. `/next-pool` is the only command allowed to advance the active pool.
...
2. Verify that all nine required files exist:
...
| `/cicd-setup` | `cicd-pipeline-generator` | After approved collection artifacts; does not advance a pool stage |
...
3. Replace `$ARGUMENTS` with the requested values. For example, the logical invocation `/domain-test pool-a email` means using `.agents/commands/domain-test.md` with `$ARGUMENTS` set to `pool-a email`.
...
flowchart TD
    ...
    N --> A
    M -- Yes --> O[Optional /cicd-setup and final submission review]
...
2. **Generate Stage 1 cases:** run every technique required for that pool, one template at a time. `/domain-test` accepts one field per invocation and may be repeated for additional fields. Do not mark Stage 1 DONE until all applicable technique outputs have been reviewed and confirmed.
...
10. **Prepare CI/CD proposal:** once approved collection artifacts are available, `/cicd-setup` may propose the workflow and two-run report template. It does not change pool progress and must not trigger a workflow.

### 6.4 Pool-specific command order

| Pool | Stage 1 command order | Common continuation |
|---|---|---|
| Pool A | `/domain-test pool-a <field>` as needed → `/state-transition pool-a` → `/security-check pool-a` | `/audit pool-a` → `/extend pool-a` → `/build-postman pool-a` → `/bug-report pool-a` → `/next-pool` |
| Pool B | `/domain-test pool-b <field>` as needed → `/decision-table pool-b` → `/security-check pool-b` | `/audit pool-b` → `/extend pool-b` → `/build-postman pool-b` → `/bug-report pool-b` → `/next-pool` |
| Pool C | `/domain-test pool-c <field>` as needed → `/security-check pool-c` | `/audit pool-c` → `/extend pool-c` → `/build-postman pool-c` → `/bug-report pool-c`; no successor pool |
...
Use `DONE`, `IN_PROGRESS`, and `NOT_STARTED` for pool status. Preserve completed pools and audit history. When advancing A → B or B → C, set the next pool to `IN_PROGRESS` with all five stages unchecked. Pool C has no successor.
